...
properties = {
  ...
  hasToolChanger:false, // dinfines if machine has an ATC
...
var dwellOutput = createVariable({prefix:"D"}, feedFormat);
var feedOutput = createVariable({prefix:"F"}, feedFormat);
var pitchOutput = createVariable({prefix:"Q", force:true}, pitchFormat);
 ...
 */
  ...
  gCycleModal.format(80),gFormat.format(40),gFormat.format(49), gFeedModeModal.format(94));
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    if (insertToolCall && properties.hasToolChanger) {
    ...
    writeBlock(gFormat.format(0), zOutput.format(initialPosition.z)); // retract to initial posistion of operation G28 not possible
    zOutput.reset();
   }
 ...
 */
  ...
  if (insertToolCall ||
    ...
    writeBlock("S" + tool.spindleRPM);
  }

/*   // most routers in this range do not have the ability to zero the machine so wfo are not possible
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "deep-drilling":
      ...
      break;
    case "tapping": // we use the Velocity CNC Advanced Tapping Cycle
      tappingActiv = true
      if (!F) {
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract),
        pitchOutput.format(tool.threadPitch), "S" + tool.spindleRPM, dwellOutput.format(P) // always output S and Q manual is incorrect
      );
      break;
    case "left-tapping":
      tappingActiv = true
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract),
        pitchOutput.format(tool.threadPitch), "S" + tool.spindleRPM, dwellOutput.format(P)
      );
      break;
    case "right-tapping":
      tappingActiv = true
    ...
    if (tappingActiv) {
      writeBlock(_x, _y, pitchOutput.format(tool.threadPitch), "S" + tool.spindleRPM);
     } else {
       writeBlock(_x, _y);
     }
    }
...
function onClose() {
  ...
  if (properties.hasToolChanger) {
     writeBlock("T0", mFormat.format(6));
  } else {  
    onCommand(COMMAND_STOP_SPINDLE);
 }
